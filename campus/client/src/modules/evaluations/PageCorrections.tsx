...
export default function PageCorrections() {
  ...
  const parEcheance = (a: DevoirEnseignantResume, b: DevoirEnseignantResume) => a.dateLimite.localeCompare(b.dateLimite);
  // Deux attentes distinctes : des copies sans note (à corriger), des notes posées pas encore publiées (à publier).
  const aCorriger = devoirs.filter((d) => d.type === "depot" && d.compteurs.aCorriger > 0).sort(parEcheance);
  const aPublier = devoirs.filter((d) => d.type === "depot" && d.compteurs.aCorriger === 0 && d.compteurs.aPublier > 0).sort(parEcheance);
  const ids = new Set([...aCorriger, ...aPublier].map((d) => d.id));
  const enCours = devoirs.filter((d) => !ids.has(d.id) && new Date(d.dateLimite).getTime() >= maintenant).sort(parEcheance);
  const termines = devoirs.filter((d) => !ids.has(d.id) && new Date(d.dateLimite).getTime() < maintenant);
  const totalACorriger = devoirs.reduce((s, d) => s + (d.type === "depot" ? d.compteurs.aCorriger : 0), 0);
  const totalAPublier = devoirs.reduce((s, d) => s + (d.type === "depot" ? d.compteurs.aPublier : 0), 0);
  const resume = [
    totalACorriger ? `${pluriel(totalACorriger, "copie")} à corriger` : null,
    totalAPublier ? `${pluriel(totalAPublier, "note")} à publier` : null,
  ].filter(Boolean);
  ...
  return (
    <Page className="max-w-5xl">
      <EnTetePage
        ...
        titre="Corrections"
        sousTitre={data ? (resume.length ? `${resume.join(" · ")}.` : "Aucune copie en attente de correction.") : undefined}
        actions={
      ...
      {isLoading ? (
        ...
        <>
          ...
          {aPublier.length > 0 && (
            <section>
              <TitreSection titre="Notes à publier" />
              <div className="flex flex-col gap-3">
                {aPublier.map((d) => (
                  <LigneDevoirEnseignant key={d.id} d={d} maintenant={maintenant} />
                ))}
